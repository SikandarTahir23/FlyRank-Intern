...
// Stage 3: adds update (PUT) and delete (DELETE) endpoints, backed by a
// parameterized UPDATE and DELETE FROM. Routes, status codes, validation,
// and response bodies are unchanged from the original in-memory version.
...
//   We validate before touching the database so bad requests never mutate
//   data.
...
router.put("/tasks/:id", (request, response) => {
  ...
  // Validation passed but no row matched -> 404 Not Found.
  if (updatedTask === null) {
    return response.status(404).json({
      error: `Task with id ${taskId} was not found`,
    });
...
router.delete("/tasks/:id", (request, response) => {
  ...
  // No row with this id existed -> 404 Not Found.
  if (!wasDeleted) {
    return response.status(404).json({
      error: `Task with id ${taskId} was not found`,
    });
