 ...
 * - Known application errors (custom status codes)
 * - Unexpected errors (500, logged for debugging)
 * 
...
export function errorMiddleware(
  ...
  // Unexpected errors - log full details but don't expose to client
  ...
  sendError(
    ...
    500,
    isDevelopment ? { message: error.message, stack: error.stack } : undefined
  );
}

/**
 * Creates an operational error with a specific status code.
 * Use this for expected error conditions (not bugs).
 */
...
/**
 * Common operational error creators
 */
export const Errors = {
  ...
  rateLimited: (retryAfter: number) => {
    const error = new OperationalError('Rate limit exceeded', 429, 'RATE_LIMITED');
    (error as any).retryAfter = retryAfter;
    return error;
  },
  
  payloadTooLarge: (maxBytes: number) => 
    new OperationalError(`Payload exceeds ${maxBytes} bytes`, 413, 'PAYLOAD_TOO_LARGE'),
};